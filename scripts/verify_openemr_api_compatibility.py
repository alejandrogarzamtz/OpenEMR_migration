...
assert len(parity)==103 and len(standard)==98 and len(portal)==5
...
for token in ("test_standard_api_core_contract_and_legacy_envelope","test_standard_api_clinical_resources_and_encounter_notes","test_standard_api_administration_and_reference_catalogs","test_standard_api_patient_documents_employer_and_insurance","test_standard_api_patient_messages_and_transactions","test_standard_api_global_clinical_catalogs_and_background_services","test_portal_compatibility_contract_is_patient_isolated"):assert token in tests,f"OpenEMR compatibility test missing: {token}"
