...
| Patient demographics | Partial | normalized contacts, related people, employment, previous names and versioned consent/directive history have audited React workflows; every storable DEM `layout_options` definition and patient value migrates idempotently with type, group, list choices, validation metadata and original values, while already-normalized fields are hidden from the advanced editor; complete field-level reconciliation, guardian-column backfill and remaining advanced editing remain |
