...
| Patient demographics | Partial | normalized multiple addresses, telecoms, related people and employment history preserve priority, validity, primary/emergency status, medical-information/decision flags and historical periods with audited React workflows; structured previous names are searchable; eleven legacy consent/directive fields migrate into versioned, revocable decisions that synchronize operational communication/portal flags; advanced editing, site-specific custom fields and complete legacy reconciliation remain |
