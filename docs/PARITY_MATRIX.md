...
| Users, practitioners and roles | Partial | audited invitation, activation/deactivation, session revocation, explicit role/grant editing and last-administrator protection exist; full legacy ACL/ACO graph migration and practitioner preferences remain |
...
| Templates and layouts | Partial | advanced demographics already use persisted layout definitions; versioned localized content templates now provide declared-variable validation, escaped preview, immutable history and provenance-preserving legacy document-template import; remaining specialized form layouts and legal delivery/signing policy remain |
...
| Localization | Partial | persistent catalogs, custom translations, default locale, date format, LTR/RTL React shell behavior and lossless legacy language import exist; remaining feature-level strings and locale-aware currency/number formatting remain |
| Administration/configuration | Partial | audited facilities, staff lifecycle, safe allowlisted application settings, lists/codes, extension lifecycle, background workers and backup/restore operations exist; arbitrary legacy globals are intentionally not exposed, while organization hierarchy and remaining specialized settings remain |
| Webhooks/integration events | Complete | durable versioned events, exact/wildcard subscriptions, encrypted secrets, HMAC-SHA256 delivery, pre-delivery public-address checks, redirect rejection, continuous concurrent-safe processing, exponential retry, dead-letter replay, administration UI and contract tests are implemented; production network egress policy and external receiver acceptance are site deployment evidence |
| Extension/module compatibility | Complete | strict manifests, install/update/enable/disable lifecycle, hashed rotating credentials, namespaced event publication, Python SDK and React administration are implemented; legacy module/hook/ACL inventory is imported with configuration values hashed rather than exposed, and PHP modules require an explicit out-of-process adapter rather than unsafe binary execution |
| Security/compliance operations | Complete | application-owned controls include transactional audit seals, fail-closed production configuration, hardened containers, exact schema/key readiness, continuous delivery processing, encrypted checksummed backups, empty-target restore protection, reconciliation/cutover evidence, acceptance/load checks, CodeQL and attested release images; site-specific external anchoring, key rotation ceremony, retention, DR timing, access reviews and regulatory approval must be evidenced by each deploying organization |
