...
| Administration/configuration | Partial | audited facilities, staff lifecycle, safe allowlisted application settings, lists/codes and background-service registry exist; arbitrary legacy globals are intentionally not exposed, while modules belong to the extension phase and backups to production operations |
| Webhooks/integration events | Complete | durable versioned events, exact/wildcard subscriptions, encrypted secrets, HMAC-SHA256 delivery, pre-delivery public-address checks, redirect rejection, exponential retry, dead-letter replay, administration UI and contract tests are implemented; continuous worker operation, network egress enforcement and external receiver acceptance remain phase 7 deployment gates |
