...
| Standard OpenEMR API | Partial | all 5 legacy portal routes and 24 Standard API routes for facilities, patients, encounters, practitioners, appointments and version/product discovery have authenticated, audited compatibility contracts and the legacy response envelope; remaining route state is explicit in `LEGACY_API_INVENTORY.md` |
