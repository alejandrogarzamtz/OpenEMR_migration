...
def metadata():
    ...
    patient_resources.extend([{"type":name,"interaction":read_search,"searchParam":[{"name":"patient","type":"reference"}]} for name in ("Media","MedicationDispense","Specimen","Procedure")])
    patient_resources.extend([{"type":"Device","interaction":read_search},{"type":"Medication","interaction":read_search,"searchParam":[{"name":"code","type":"token"}]}])
    patient_resources.extend([{"type":"Provenance","interaction":read_search,"searchParam":[{"name":"target","type":"reference"}]},{"type":"ValueSet","interaction":read_search},{"type":"Group","interaction":read_search},{"type":"PractitionerRole","interaction":read_search},{"type":"OperationDefinition","interaction":read_search}])
    resources=[{"type":"Patient","interaction":[*read_search,{"code":"create"},{"code":"update"}],"searchParam":[{"name":"_id","type":"token"},{"name":"family","type":"string"},{"name":"given","type":"string"}]},*patient_resources,*status_resources,*directory_resources]
    for resource in resources:
        if resource["type"] in {"Organization","Practitioner"}:resource["interaction"]=[*read_search,{"code":"create"},{"code":"update"}]
