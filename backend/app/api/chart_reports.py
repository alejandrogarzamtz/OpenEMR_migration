...
def printable_patient_report(patient_uuid:str,start:date|None=Query(default=None),end:date|None=Query(default=None),db:Session=Depends(get_db),user:User=Depends(patient_report_user)):
    ...
    section_html="".join(f"<section><h2>{title}</h2>{content}</section>" for title,content in sections)
    ...
    db.add(AuditEvent(actor_id=user.id,action="export",resource_type="patient_report",resource_id=patient.uuid,detail=f"sha256={digest}; start={start}; end={end}; counts={evidence['counts']}"));db.commit()
    return Response(html,media_type="text/html",headers={"Cache-Control":"private, no-store","Content-Disposition":f'inline; filename="patient-{patient.uuid}-report.html"',"Content-Security-Policy":"default-src 'none'; style-src 'unsafe-inline'","X-Content-Type-Options":"nosniff","X-Report-SHA256":digest})
