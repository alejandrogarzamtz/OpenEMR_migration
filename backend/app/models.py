...
class BulkExportJob(Base):
    ...
    error: Mapped[list] = mapped_column(JSON, default=list)
...
class InventoryProduct(Base):
    ...
    cyp_factor: Mapped[Decimal] = mapped_column(Numeric(8, 4), default=0)
...
class LabOrder(Base):
    ...
    activity: Mapped[bool] = mapped_column(default=True, index=True)
    provider_legacy_id: Mapped[int | None] = mapped_column(nullable=True)
    lab_legacy_id: Mapped[int | None] = mapped_column(nullable=True)
    specimen_type: Mapped[str | None] = mapped_column(String(31), nullable=True)
    specimen_location: Mapped[str | None] = mapped_column(String(31), nullable=True)
    specimen_volume: Mapped[str | None] = mapped_column(String(30), nullable=True)
    clinical_history: Mapped[str | None] = mapped_column(String(255), nullable=True)
    external_id: Mapped[str | None] = mapped_column(String(20), nullable=True)
    order_diagnosis: Mapped[str | None] = mapped_column(String(255), nullable=True)
    procedure_order_type: Mapped[str | None] = mapped_column(String(32), nullable=True)
    legacy_payload: Mapped[dict | None] = mapped_column(JSON, nullable=True)


class ProcedureOrderLine(Base):
    __tablename__ = "procedure_order_lines"
    __table_args__ = (UniqueConstraint("order_id", "sequence", name="uq_procedure_order_line_sequence"),)
    id: Mapped[int] = mapped_column(primary_key=True)
    uuid: Mapped[str] = mapped_column(String(36), unique=True, default=lambda: str(uuid4()), index=True)
    ...
    source: Mapped[str | None] = mapped_column(String(1), nullable=True)
    ...
    reason_date_low: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    reason_date_high: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    reason_status: Mapped[str | None] = mapped_column(String(31), nullable=True)
    legacy_payload: Mapped[dict | None] = mapped_column(JSON, nullable=True)


class ProcedureReport(Base):
    __tablename__ = "procedure_reports"
    id: Mapped[int] = mapped_column(primary_key=True)
    uuid: Mapped[str] = mapped_column(String(36), unique=True, default=lambda: str(uuid4()), index=True)
    legacy_report_id: Mapped[int | None] = mapped_column(unique=True, nullable=True)
    order_id: Mapped[int | None] = mapped_column(ForeignKey("lab_orders.id"), nullable=True, index=True)
    legacy_order_id: Mapped[int | None] = mapped_column(nullable=True, index=True)
    order_sequence: Mapped[int] = mapped_column(default=1)
    collected_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    collected_timezone: Mapped[str | None] = mapped_column(String(5), nullable=True)
    reported_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    reported_timezone: Mapped[str | None] = mapped_column(String(5), nullable=True)
    source_legacy_user_id: Mapped[int | None] = mapped_column(nullable=True)
    specimen_number: Mapped[str | None] = mapped_column(String(63), nullable=True)
    status: Mapped[str | None] = mapped_column(String(31), nullable=True)
    review_status: Mapped[str | None] = mapped_column(String(31), nullable=True)
    notes: Mapped[str | None] = mapped_column(Text, nullable=True)
    legacy_payload: Mapped[dict | None] = mapped_column(JSON, nullable=True)
...
class LabResult(Base):
    ...
    order_id: Mapped[int | None] = mapped_column(ForeignKey("lab_orders.id"), nullable=True, index=True)
    report_id: Mapped[int | None] = mapped_column(ForeignKey("procedure_reports.id"), nullable=True, index=True)
    ...
    data_type: Mapped[str | None] = mapped_column(String(1), nullable=True)
    facility: Mapped[str | None] = mapped_column(String(255), nullable=True)
    comments: Mapped[str | None] = mapped_column(Text, nullable=True)
    legacy_document_id: Mapped[int | None] = mapped_column(nullable=True)
    ended_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    legacy_payload: Mapped[dict | None] = mapped_column(JSON, nullable=True)
...
class ReceivableActivity(Base):
    ...
    legacy_session_id: Mapped[int | None] = mapped_column(nullable=True, index=True)
...
class ServiceCode(Base):
    ...
    financial_reporting: Mapped[bool]=mapped_column(default=False,index=True)
    cyp_factor: Mapped[Decimal]=mapped_column(Numeric(8,4),default=0)
...
class Pharmacy(Base):
    ...
    email: Mapped[str | None] = mapped_column(String(255), nullable=True)
    transmit_method: Mapped[int] = mapped_column(default=1)
    legacy_payload: Mapped[dict | None] = mapped_column(JSON, nullable=True)
...
class Prescription(Base):
    ...
    patient_id: Mapped[int | None] = mapped_column(ForeignKey("patients.id"), nullable=True, index=True)
    legacy_patient_id: Mapped[int | None] = mapped_column(nullable=True, index=True)
    encounter_id: Mapped[int | None] = mapped_column(ForeignKey("encounters.id"), nullable=True)
    ...
    status: Mapped[str] = mapped_column(String(30), default="active")
    filled_by_legacy_id: Mapped[int | None] = mapped_column(nullable=True)
    provider_legacy_id: Mapped[int | None] = mapped_column(nullable=True)
    drug_legacy_id: Mapped[int | None] = mapped_column(nullable=True)
    form_legacy_id: Mapped[int | None] = mapped_column(nullable=True)
    dosage: Mapped[str | None] = mapped_column(String(100), nullable=True)
    size: Mapped[str | None] = mapped_column(String(25), nullable=True)
    unit_legacy_id: Mapped[int | None] = mapped_column(nullable=True)
    route: Mapped[str | None] = mapped_column(String(100), nullable=True)
    interval_legacy_id: Mapped[int | None] = mapped_column(nullable=True)
    per_refill: Mapped[int | None] = mapped_column(nullable=True)
    filled_date: Mapped[date | None] = mapped_column(Date, nullable=True)
    medication_legacy_id: Mapped[int | None] = mapped_column(nullable=True)
    note: Mapped[str | None] = mapped_column(Text, nullable=True)
    legacy_recorded_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    legacy_user: Mapped[str | None] = mapped_column(String(50), nullable=True)
    site: Mapped[str | None] = mapped_column(String(50), nullable=True)
    prescription_guid: Mapped[str | None] = mapped_column(String(50), nullable=True)
    erx_source: Mapped[int] = mapped_column(default=0)
    erx_uploaded: Mapped[bool] = mapped_column(default=False)
    erx_drug_info: Mapped[str | None] = mapped_column(Text, nullable=True)
    external_id: Mapped[str | None] = mapped_column(String(20), nullable=True)
    prn: Mapped[str | None] = mapped_column(String(30), nullable=True)
    ntx: Mapped[int | None] = mapped_column(nullable=True)
    rtx: Mapped[int | None] = mapped_column(nullable=True)
    transaction_date: Mapped[date | None] = mapped_column(Date, nullable=True)
    usage_category: Mapped[str | None] = mapped_column(String(100), nullable=True)
    usage_category_title: Mapped[str | None] = mapped_column(String(255), nullable=True)
    request_intent: Mapped[str | None] = mapped_column(String(100), nullable=True)
    request_intent_title: Mapped[str | None] = mapped_column(String(255), nullable=True)
    diagnosis: Mapped[str | None] = mapped_column(Text, nullable=True)
    created_by_legacy_id: Mapped[int | None] = mapped_column(nullable=True)
    updated_by_legacy_id: Mapped[int | None] = mapped_column(nullable=True)
    legacy_payload: Mapped[dict | None] = mapped_column(JSON, nullable=True)
