...
def test_patient_flow():
    with TestClient(app) as client:
        ...
        result = client.post(f"/api/v1/lab-orders/{order.json()['uuid']}/results", headers=headers, json={"observed_at": "2026-09-02T11:00:00Z", "code": "718-7", "name": "Hemoglobin", "value": "13.4", "unit": "g/dL", "reference_range": "12-16", "status": "final", "facility": "Central Lab", "comments": "Verified specimen"})
        assert result.status_code == 201
        detail = client.get(f"/api/v1/lab-orders/{order.json()['uuid']}", headers=headers).json()
        assert detail["lines"][0] == {**detail["lines"][0], "sequence": 1, "code": "718-7", "name": "Hemoglobin"}
        assert detail["results"][0]["value"] == "13.4"
        assert detail["results"][0]["facility"] == "Central Lab"
        assert detail["results"][0]["comments"] == "Verified specimen"
