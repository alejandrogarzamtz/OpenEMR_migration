...
def test_report_catalog_snapshots_filters_checksums_and_csv_export():
    with TestClient(app) as client:
        ...
        assert {item["key"]:item["migration_status"] for item in catalog.json() if item["key"] in {"criteria.tab","report.script"}}=={"criteria.tab":"embedded","report.script":"embedded"}
        assert client.post("/api/v1/reports/criteria.tab/runs",headers=headers,json={}).status_code==501
