...
def test_printable_chart_report_is_patient_scoped_escaped_audited_and_permissioned():
    with TestClient(app) as client:
        ...
        history=client.post(f"/api/v1/patients/{patient['uuid']}/social-history",headers=admin,json={"recorded_at":"2026-09-14T12:00:00Z","tobacco":"Never","exercise_patterns":"Walks daily"})
        ...
        assert all(section in response.text for section in ("Demographics","Problems, allergies, and medications","Social history","Never","Encounters","Care plans","Walk daily","Laboratory and procedures","Documents","Insurance","Claims"))
