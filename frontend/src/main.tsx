...
import { uiCopy, uiLanguage } from "./i18n";
...
type Prescription = { uuid:string; drug_name:string; dosage_instructions:string; dosage?:string; route?:string; quantity?:string; refills:number; status:string; prescribed_at?:string; modified_at?:string; pharmacy_name?:string; indication?:string; diagnosis?:string; prn?:string; erx_source:number; erx_uploaded:boolean; usage_category_title?:string; request_intent_title?:string };
...
function App(){
  ...
  const language=uiLanguage(localization?.locale.code);const copy=(spanish:string,english:string)=>uiCopy(language,spanish,english);
  ...
  return <div className="shell">
    <aside className="app-sidebar"><div className="brand-lockup"><div className="brand">OR</div><div><strong>{localization?.organization_name||"OpenRM"}</strong><small>{copy("Espacio clínico","Clinical workspace")}</small></div></div><nav aria-label={copy("Navegación principal","Primary navigation")}><button className={section==="patients"?"active":""} onClick={()=>setSection("patients")}><span aria-hidden="true">01</span>{t("nav.patients",copy("Pacientes","Patients"))}</button><button className={section==="appointments"?"active":""} onClick={()=>setSection("appointments")}><span aria-hidden="true">02</span>{t("nav.appointments",copy("Agenda","Appointments"))}</button><button className={section==="flow"?"active":""} onClick={()=>setSection("flow")}><span aria-hidden="true">03</span>{t("nav.flow",copy("Flujo de pacientes","Patient flow"))}</button><button className={section==="inventory"?"active":""} onClick={()=>setSection("inventory")}><span aria-hidden="true">04</span>{t("nav.inventory",copy("Inventario","Inventory"))}</button><button className={section==="communications"?"active":""} onClick={()=>setSection("communications")}><span aria-hidden="true">05</span>{t("nav.messages",copy("Mensajes","Messages"))}</button><button className={section==="education"?"active":""} onClick={()=>setSection("education")}><span aria-hidden="true">06</span>{t("nav.education",copy("Educación","Education"))}</button><button className={section==="reports"?"active":""} onClick={()=>setSection("reports")}><span aria-hidden="true">07</span>{t("nav.reports",copy("Reportes","Reports"))}</button><button className={section==="administration"?"active":""} onClick={()=>setSection("administration")}><span aria-hidden="true">08</span>{t("nav.administration",copy("Administración","Administration"))}</button><button className={section==="security"?"active":""} onClick={()=>setSection("security")}><span aria-hidden="true">09</span>{t("nav.security",copy("Seguridad","Security"))}</button></nav><div className="sidebar-footer">{localization&&<label><span>{copy("Idioma","Language")}</span><select aria-label={copy("Idioma de la interfaz","Interface language")} value={localization.locale.code} onChange={event=>chooseLocale(event.target.value)}>{localization.locales.map(item=><option key={item.code} value={item.code}>{item.name}</option>)}</select></label>}<button className="logout" onClick={()=>void logout()}>{t("action.logout",copy("Salir","Sign out"))}</button></div></aside>
    <main className="app-main"><header className="app-header"><div><p className="eyebrow">{t("header.clinical_care",copy("ATENCIÓN CLÍNICA","CLINICAL CARE"))}</p><h1>{section==="patients"?t("nav.patients",copy("Pacientes","Patients")):section==="appointments"?t("nav.appointments",copy("Agenda","Appointments")):section==="flow"?t("nav.flow",copy("Flujo de pacientes","Patient flow")):section==="inventory"?t("nav.inventory",copy("Inventario","Inventory")):section==="communications"?t("nav.messages",copy("Mensajes seguros","Secure messages")):section==="education"?t("nav.education",copy("Educación del paciente","Patient education")):section==="security"?t("nav.security",copy("Seguridad","Security")):section==="reports"?t("nav.reports",copy("Reportes","Reports")):t("nav.administration",copy("Administración","Administration"))}</h1><p className="page-description">{section==="patients"?copy("Expedientes, atención clínica y seguimiento en un solo lugar.","Records, clinical care, and follow-up in one place."):section==="appointments"?copy("Organiza citas, recursos y llegadas de pacientes.","Organize appointments, resources, and patient arrivals."):section==="flow"?copy("Supervisa en tiempo real el recorrido de cada paciente.","Monitor each patient's journey in real time."):section==="inventory"?copy("Controla productos, lotes y existencias disponibles.","Manage products, lots, and available stock."):section==="communications"?copy("Coordina conversaciones seguras con pacientes y equipos.","Coordinate secure conversations with patients and teams."):section==="education"?copy("Consulta recursos educativos para apoyar cada plan de atención.","Find education resources that support every care plan."):section==="reports"?copy("Explora y exporta información clínica y operativa.","Explore and export clinical and operational information."):section==="security"?copy("Protege tu cuenta y administra la autenticación multifactor.","Protect your account and manage multifactor authentication."):copy("Configura la organización, accesos e integraciones.","Configure the organization, access, and integrations.")}</p></div>{section==="patients"&&<button className="primary-action" onClick={()=>setCreatingPatient(true)}><span aria-hidden="true">＋</span>{t("action.new_patient",copy("Nuevo paciente","New patient"))}</button>}</header>
      {error&&<p className="error">{error}</p>}
      {section==="appointments"?<AppointmentBoard api={api} patients={patients} language={language}/>:section==="flow"?<PatientFlowBoard api={api} language={language}/>:section==="inventory"?<InventoryWorkspace api={api} language={language}/>:section==="communications"?<CommunicationWorkspace api={api} patients={patients} language={language}/>:section==="education"?<PatientEducationWorkspace api={api} language={language}/>:section==="security"?<SecurityWorkspace api={api} language={language}/>:section==="administration"?<AdministrationWorkspace api={api} language={language}/>:section==="reports"?<ReportWorkspace api={api} language={language}/>:<>
      {creatingPatient&&<PatientForm onSubmit={createPatient} onCancel={()=>setCreatingPatient(false)}/>}
      <div className={selected?"workspace detail-open":"workspace"}><section className="card patient-list-panel">
        <div className="toolbar"><input aria-label={copy("Buscar pacientes","Search patients")} placeholder={copy("Buscar por nombre o correo","Search by name or email")} value={query} onChange={event=>setQuery(event.target.value)} onKeyDown={event=>event.key==="Enter"&&void loadPatients()}/><span>{patients.length} {copy("resultados","results")}</span></div>
        <table><thead><tr><th>{copy("Paciente","Patient")}</th><th>{copy("Fecha de nacimiento","Date of birth")}</th><th>{copy("Sexo","Sex")}</th><th>{copy("Correo","Email")}</th></tr></thead><tbody>{patients.map(patient=><tr className="patient-row" key={patient.uuid} onClick={()=>void openPatient(patient)}><td><strong>{patient.last_name}, {patient.first_name}</strong><small>{patient.uuid.slice(0,8)}</small></td><td>{patient.date_of_birth}</td><td>{patient.sex}</td><td>{patient.email??"—"}</td></tr>)}</tbody></table>
        {!patients.length&&<div className="empty">{copy("No hay pacientes que mostrar.","No patients to display.")}</div>}
      </section>{selected&&<aside className="patient-drawer">
        <div className="patient-detail-heading"><div><p className="eyebrow">EXPEDIENTE DEL PACIENTE</p><h2>{selected.patient.first_name} {selected.patient.last_name}</h2><p>{selected.patient.date_of_birth} · {selected.patient.sex}</p></div><button className="close" aria-label="Cerrar expediente" onClick={()=>setSelected(null)}>×</button></div>
        ...
        <form className="quick-add file-upload-form" onSubmit={uploadDocument}><label>Seleccionar documento<input name="file" type="file" required/></label><button>Subir documento</button></form>
