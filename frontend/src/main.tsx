...
function App(){
  ...
  return <div className="shell">
    <aside className="app-sidebar"><div className="brand-lockup"><div className="brand">OR</div><div><strong>{localization?.organization_name||"OpenRM"}</strong><small>Clinical workspace</small></div></div><nav aria-label="Navegación principal"><button className={section==="patients"?"active":""} onClick={()=>setSection("patients")}><span aria-hidden="true">01</span>{t("nav.patients","Pacientes")}</button><button className={section==="appointments"?"active":""} onClick={()=>setSection("appointments")}><span aria-hidden="true">02</span>{t("nav.appointments","Agenda")}</button><button className={section==="flow"?"active":""} onClick={()=>setSection("flow")}><span aria-hidden="true">03</span>{t("nav.flow","Flujo")}</button><button className={section==="inventory"?"active":""} onClick={()=>setSection("inventory")}><span aria-hidden="true">04</span>{t("nav.inventory","Inventario")}</button><button className={section==="communications"?"active":""} onClick={()=>setSection("communications")}><span aria-hidden="true">05</span>{t("nav.messages","Mensajes")}</button><button className={section==="education"?"active":""} onClick={()=>setSection("education")}><span aria-hidden="true">06</span>{t("nav.education","Educación")}</button><button className={section==="reports"?"active":""} onClick={()=>setSection("reports")}><span aria-hidden="true">07</span>{t("nav.reports","Reportes")}</button><button className={section==="administration"?"active":""} onClick={()=>setSection("administration")}><span aria-hidden="true">08</span>{t("nav.administration","Administración")}</button><button className={section==="security"?"active":""} onClick={()=>setSection("security")}><span aria-hidden="true">09</span>{t("nav.security","Seguridad")}</button></nav><div className="sidebar-footer">{localization&&<label><span>Idioma</span><select aria-label="Interface language" value={localization.locale.code} onChange={event=>chooseLocale(event.target.value)}>{localization.locales.map(item=><option key={item.code} value={item.code}>{item.name}</option>)}</select></label>}<button className="logout" onClick={()=>void logout()}>{t("action.logout","Cerrar sesión")}</button></div></aside>
    <main className="app-main"><header className="app-header"><div><p className="eyebrow">{t("header.clinical_care","ATENCIÓN CLÍNICA")}</p><h1>{section==="patients"?t("nav.patients","Pacientes"):section==="appointments"?t("nav.appointments","Agenda"):section==="flow"?t("nav.flow","Flujo de pacientes"):section==="inventory"?t("nav.inventory","Inventario"):section==="communications"?t("nav.messages","Mensajes seguros"):section==="education"?t("nav.education","Educación del paciente"):section==="security"?t("nav.security","Seguridad"):section==="reports"?t("nav.reports","Reportes"):t("nav.administration","Administración")}</h1><p className="page-description">{section==="patients"?"Expedientes, atención clínica y seguimiento en un solo lugar.":section==="appointments"?"Organiza citas, recursos y llegadas de pacientes.":section==="flow"?"Supervisa en tiempo real el recorrido de cada paciente.":section==="inventory"?"Controla productos, lotes y existencias disponibles.":section==="communications"?"Coordina conversaciones seguras con pacientes y equipos.":section==="education"?"Consulta recursos educativos para apoyar cada plan de atención.":section==="reports"?"Explora y exporta información clínica y operativa.":section==="security"?"Protege tu cuenta y administra la autenticación multifactor.":"Configura la organización, accesos e integraciones."}</p></div>{section==="patients"&&<button className="primary-action" onClick={()=>setCreatingPatient(true)}><span aria-hidden="true">＋</span>{t("action.new_patient","Nuevo paciente")}</button>}</header>
