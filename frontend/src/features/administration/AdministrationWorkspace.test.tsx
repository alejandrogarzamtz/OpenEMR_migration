...
describe("AdministrationWorkspace",()=>{
  ...
  it("registers an asymmetric SMART backend client",async()=>{
    const api=vi.fn();[[],[],[],[],[]].forEach(value=>api.mockResolvedValueOnce(value));api.mockResolvedValueOnce({uuid:"s2"});[[],[],[],[],[]].forEach(value=>api.mockResolvedValueOnce(value));render(<AdministrationWorkspace api={api}/>);await screen.findByText("SMART applications");fireEvent.change(screen.getByPlaceholderText("Client name"),{target:{value:"Warehouse analytics"}});fireEvent.change(screen.getByLabelText("Public JWKS"),{target:{value:'{"keys":[{"kty":"RSA","kid":"one"}]}'}});fireEvent.change(screen.getByLabelText("Allowed SMART scopes"),{target:{value:"system/Patient.rs system/Observation.rs"}});fireEvent.click(screen.getByText("Register SMART application"));await waitFor(()=>expect(api).toHaveBeenCalledTimes(11));const request=JSON.parse(String(api.mock.calls[5][1].body));expect(request.allowed_scopes).toEqual(["system/Patient.rs","system/Observation.rs"]);expect(request.jwks.keys[0].kid).toBe("one");expect(request.client_kind).toBe("backend");
  });
});
