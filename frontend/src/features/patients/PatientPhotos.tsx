...
export function PatientPhotos({api,patientUuid}:{api:ApiRequest;patientUuid:string}){
  ...
  return <section className="summary-group patient-photos"><h3>Fotografía<span>{photos.filter(photo=>photo.active).length}</span></h3>{imageUrl?<img className="patient-photo" src={imageUrl} alt="Fotografía principal del paciente"/>:<p>Sin fotografía activa.</p>}<label className="wide-action">Subir nueva versión<input aria-label="Subir fotografía del paciente" type="file" accept="image/jpeg,image/png,image/bmp" onChange={event=>void upload(event)} hidden/></label>{error&&<p className="error">{error}</p>}{photos.map(photo=><article key={photo.uuid}><strong>{photo.original_name}{photo.is_primary?" · principal":""}</strong><small>{photo.mime_type} · {(photo.size_bytes/1024).toFixed(1)} KiB · {photo.active?"activa":`inactiva${photo.inactivated_reason?` (${photo.inactivated_reason})`:""}`}</small>{photo.active&&!photo.is_primary&&<button className="text-button" onClick={()=>void action(photo,"primary")}>Usar como principal</button>}{photo.active&&<button className="text-button" onClick={()=>void action(photo,"inactivate")}>Inactivar versión</button>}</article>)}</section>;
}
