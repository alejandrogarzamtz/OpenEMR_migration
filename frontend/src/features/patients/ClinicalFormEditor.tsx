...
export function ClinicalFormEditor({api,patientUuid,encounterUuid,onSaved}:{api:ApiRequest;patientUuid:string;encounterUuid:string;onSaved:()=>void|Promise<void>}){
  ...
  async function submit(event:FormEvent<HTMLFormElement>){
    ...
    try{
      ...
      else if(definition?.kind==="structured")content=Object.fromEntries((definition.fields??[]).map(field=>[field.key,field.type==="boolean"?data.get(field.key)==="on":field.type==="integer"?Number(data.get(field.key)):String(data.get(field.key)??"")]));
  ...
  return <form className="clinical-form-editor" onSubmit={submit}>
    ...
    {definition?.kind==="structured"&&<div className="structured-form-grid">{(definition.fields??[]).map(field=><label key={field.key}>{field.type==="boolean"?<><input name={field.key} type="checkbox"/> {field.label}</>:<>{field.label}{field.type==="text"?<textarea name={field.key} required={field.required} maxLength={20_000}/>:<input name={field.key} type={field.type==="date"?"date":field.type==="integer"?"number":"text"} required={field.required} maxLength={field.type==="string"?255:undefined}/>}</>}</label>)}</div>}
