...
describe("CommunicationWorkspace",()=>{
  it("creates and replies to patient-bound secure threads",async()=>{
    ...
    const api=vi.fn((path:string,init?:RequestInit)=>Promise.resolve(path==="/api/v1/messages"?(init?[summary]:[]):path==="/api/v1/portal-representatives"?[]:path.endsWith("/replies")?{...summary,messages:[{uuid:"message-1",sender_kind:"staff",body:"Follow up",created_at:"2026-01-01T00:00:00Z"}]}:summary)) as any;
