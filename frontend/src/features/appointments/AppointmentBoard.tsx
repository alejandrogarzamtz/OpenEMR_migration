...
export function AppointmentBoard({api,patients}:{api:ApiRequest;patients:Patient[]}) {
  ...
  <section className="card"><h2>Appointments</h2>{items.length?<table><thead><tr><th>When</th><th>Patient</th><th>Resource</th><th>Status</th><th>Actions</th></tr></thead><tbody>{items.map(item=><tr key={item.uuid}><td>{new Date(item.starts_at).toLocaleString()}</td><td>{patients.find(patient=>patient.uuid===item.patient_uuid)?.last_name??item.patient_uuid.slice(0,8)}</td><td>{[item.provider_name,item.facility_name,item.room].filter(Boolean).join(" · ")||"—"}</td><td>{item.status}</td><td>{item.status!=="fulfilled"&&<button onClick={()=>void setStatus(item,"fulfilled")}>Complete</button>} {item.status!=="cancelled"&&<button onClick={()=>void setStatus(item,"cancelled")}>Cancel</button>}</td></tr>)}</tbody></table>:<div className="empty">No appointments to show.</div>}</section></section>;
}
